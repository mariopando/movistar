...
$(document).ready(function(){
    ...
    $.post('assign/getAssignDataRecording', function(data){
        var records_ = $.parseJSON(data);
        if(records_.success){
            records = records_.data;
        }
        else{
           alert('Sin asignaciones a logistica externa.')
        }
    });   
...
function template_functions(){
	...
	/* ---------- Inner table content ---------- */
        ...
        dispatch_select = '';
        $.get('assign/getDispatchers', function(data){
            dispatchs = $.parseJSON(data);
            for(i = 0; dispatchs.length > i;i++){                
                dispatch_select = dispatch_select + '<option value="'+dispatchs[i].id+'">'+dispatchs[i].name+'</option>';
            }
        }); 
        
        $('#input_assign').focus();
        $('#input_assign').bind('change',function(e){
            e.preventDefault();
            ola = $(this).val();
            is = false;
            if(ola.length > 0){
                $.grep(records,function(data){
                    if(data.id === ola && typeof data.isSee === 'undefined'){
                        data.isSee = true;
                        //input_select = '<select name="externlogistic_status"><option selected="selected" value="0">Sin logistica asignada</option><option value="1">Logistica externa 1</option><option value="2">Logistica externa 2</option><option value="3">Logistica externa 3</option><option value="4">Logistica externa 4</option></select>';
                        input_ola_id = '<input type="hidden" value="'+ola+'" />';
                        $('#input_assign_content > table > tbody').prepend('<tr><td><input type="checkbox" selected="selected" class="row_selected" /></td><td>'+ola+'</td><td>'+data.logistic_id+'</td><td>'+data.Estado+'</td><td><select name="assign_status dynamic" data-ola="'+ola+'">'+dispatch_select+'</select></td>'+input_ola_id+'</tr>');
                        $('#input_assign').val('');
                        $('#input_assign').focus();      
                        $('#input_assign_content').find('select:eq(0) > option').each(function(){
                            if(data.dispatcher_id === $(this).attr('value'))
                                $(this).attr('selected','selected');
                        });
                        
                        $('#input_assign_content select:eq(0)').bind('change',function(e){
                            $(this).parent().append('<img class="preloader" src="../img/loader1.gif"/>');
                            $.post('assign/setDispatcher',{sale_id:$(this).attr('data-ola'),value:$(this).val()}, function(data){
                                obj = $.parseJSON(data);
                                if(! obj.success){
...
//                    });
        ...
        $('#selectall_chk').bind('change',function(){
            ref = $(this);
            if(ref.is(':checked')){
                $('#input_assign_content > table > tbody').find('input[type=checkbox]').each(function(){
        ...
        $('#change_status').bind('change',function(e){
            e.preventDefault();
            ref = $(this);
            $('#input_assign_content > table > tbody').find('tr').each(function(){
                if($('input.row_selected',$(this)).is(':checked')){
                    $('select[name=assign_status]',$(this)).val(ref.val());
                }
             });            
        });
