...
//            });               
	...
	var tbl1 = $('.datatable').dataTable({
			...
			"sLengthMenu": "_MENU_ records per page"
                        ...
                        "sAjaxSource": "misc/table-data.php",
...
function init_load_images(){
    var images = JSON.parse(localStorage.getItem('images'));

    if(images){
//        console.log('LENGTH------->'+images.ola.length);
        for(i = 0; i < images.ola.length; i++){
            if(typeof images[i].ola !== 'undefined')
            $('#gallery_localstorage').prepend('<div id="image-'+ i +'" class="masonry-thumb"><a title="Image '+ i +'"><img class="grayscale" src="'+ images.src[i] +'" alt="Image 1"></a><button class="btn btn-danger crud_event_delete" data-ola="'+ images.ola[i] +'" type="button">Eliminar</button><p> Fecha: '+ images.date[i] +'</p></div>');
        }
    }  
    
    //Server Images
    $.ajax({ type: "POST", url: "localstorage/getImages", data: {user_id: user_id}
    }).done(function( data ) {
        $('#gallery_serverstorage').html('<img class="preloader" src="../img/loader1.gif"/>');
        ...
        if(! obj.success){
            ...
            for(i = 0; i < obj.images.length; i++){
                $('#gallery_serverstorage').prepend('<div id="image-'+ i +'" class="masonry-thumb"><a title="Image '+ i +'"><img class="grayscale" src="'+ obj.images[i].src +'" alt="Image 1"></a><button class="btn btn-danger crud_event_delete" data-id="' + obj.images[i].id + '" type="button">Eliminar</button><p> Fecha: '+ obj.images[i].date +'</p></div>');
            }
        }
        //init_masonry();
    }).error(function(){
        ...
        $('#gallery_serverstorage .preloader').remove();
          init_masonry();  
    });
...
function crud_events(){
    $('#gallery_serverstorage .crud_event_delete').live('click',function(){
        ref = $(this);
        $.ajax({ type: "POST", url: "localstorage/deleteImage", data: {user_id: user_id, image_id: $(this).attr('data-id')}
        }).done(function( data ) {
            obj = $.parseJSON(data);
            if(! obj.success){
                alert('Error: getImages->'+obj.msg);
            }else{
                //init_load_images();
            ...
            ref.parent().html('<span class="label label-warning">Imagen removida.</span>');
            $('#gallery_serverstorage .preloader').remove();
            init_masonry(); 
        });          
    });
    
    $('#gallery_localstorage .crud_event_delete').live('click',function(){
        ref = $(this);
        var images = JSON.parse(localStorage.getItem('images'));

        if(images){
            for(i = 0; i < images.ola.length; i++){
                if(images[i].ola === $(this).attr('data-ola')){
                    delete images[i];
                }
//                $('#gallery_localstorage').prepend('<div id="image-'+ i +'" class="masonry-thumb"><a title="Image '+ i +'"><img class="grayscale" src="'+ images.src[i] +'" alt="Image 1"></a><button class="btn btn-danger crud_event_delete" data-ola="'+ images.ola[i] +'" type="button">Eliminar</button><p> Fecha: '+ images.date[i] +'</p></div>');
            }
