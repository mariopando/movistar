...
var request_url = 'index.php/dashboard';

$(document).ready(function(){
    
    /* ---------- Login Box Styles ---------- */
    $.get('dashboard/getInfoCountries', function(data){
...
function template_functions(){
	...
	/* ---------- Datable ---------- */
        ...
        //var items = [];
        if($('.data-users')){
//            $.getJSON('misc/table-data.php', function(data) {
//                //console.log(data)
//                $.each(data, function(key, val) {
//                    //items.push(val);
//                    //items.push('<tr><td>' + val.name + '</td><td class="center">' + val.date_register + '</td><td class="center">' + val.role + '</td><td class="center"><span class="label label-success">' + val.active + '</span></td><td class="center"><a class="btn btn-success" href="#"><i class="icon-zoom-in icon-white"></i> </a><a class="btn btn-info" href="#"><i class="icon-edit icon-white"></i></a><a class="btn btn-danger" href="#"><i class="icon-trash icon-white"></i></a></td></tr>')
//                });
//                //test__ = items.join('');
//                //console.log(items)
//                //$(test).appendTo('.data-table-content');
//            });               
        }
        
        function processDetail(aoData){
            console.log(aoData)
            return true
        }
        

	var tbl1 = $('.datatable').dataTable({
			"sDom": "<'row-fluid'<'span6'l><'span6'f>r>t<'row-fluid'<'span12'i><'span12 center'p>>",
			"sPaginationType": "bootstrap",
			"oLanguage": {
			"sLengthMenu": "_MENU_ records per page"
			},
                        "sAjaxSource": "dashboard/getRecords",
                        ...
                        "fnCreatedRow": function( nRow, aData, iDataIndex ) {
                                ...
                                $('td:eq(0)', nRow).html( '<a href="#" data-ref="'+iDataIndex+'">'+aData[0]+'</a>' );
                                $('td:eq(0)', nRow).parent().addClass('viewDetail');
                                
                                main_registers = new Object();
                                main_registers.id = aData[0];
                                main_registers.franquiciado = aData[1];
                                main_registers.nombreCliente = aData[2];
                                main_registers.rutCliente = aData[3];
                                main_registers.regionEnvio = aData[4];
                                main_registers.comunaEnvio = aData[5];
                                main_registers.process = aData[6];
                                main_registers.status = aData[7];
                                main_registers.finish = aData[8];
                                array_registers['"'+aData[0].toString()+'"'] = main_registers;
                                
                                $('td:eq(0) > a', nRow).bind('mouseover',function(e){
                                    e.preventDefault();
                                    $('#box_detail span.detail_value:eq(0)').html(array_registers['"'+aData[0].toString()+'"'].id);
                                    $('#box_detail span.detail_value:eq(1)').html(array_registers['"'+aData[0].toString()+'"'].franquiciado);
                                    $('#box_detail span.detail_value:eq(2)').html(array_registers['"'+aData[0].toString()+'"'].nombreCliente);
                                    $('#box_detail span.detail_value:eq(3)').html(array_registers['"'+aData[0].toString()+'"'].rutCliente);
                                    $('#box_detail span.detail_value:eq(4)').html(array_registers['"'+aData[0].toString()+'"'].regionEnvio);
                                    $('#box_detail span.detail_value:eq(5)').html(array_registers['"'+aData[0].toString()+'"'].comunaEnvio);
                                    
                                    $('#box_detail_dispatch span.detail_value:eq(0)').html(array_registers['"'+aData[0].toString()+'"'].process);
                                    $('#box_detail_dispatch span.detail_value:eq(1)').html(array_registers['"'+aData[0].toString()+'"'].status);
                                    $('#box_detail_dispatch span.detail_value:eq(2)').html(array_registers['"'+aData[0].toString()+'"'].finish);
                                })   
                            }                                
		} );
//       console.log('------------------------------')
//       console.log(array_registers)
