...
function template_functions(){
	...
	/* ---------- Inner table content ---------- */
        ...
        $('#input_receive').bind('change',function(e){
            ...
            ola = $(this).val();
            if(ola.length > 0){
                input_select = '<select name="assign_status"><option selected="selected" value="1">Entregado</option><option value="0">Rechazado</option></select>';
                input_ola_id = '<input type="hidden" value="'+ola+'" />';
                $('#input_assign_content > table > tbody').prepend('<tr><td><input type="checkbox" selected="selected" class="row_selected" /></td><td>'+ola+'</td><td>'+input_select+'</td>'+input_ola_id+'</tr>');
                $('#input_assign').val('');
                $('#input_assign').focus();
            }else{
                $('#input_assign').val('');
