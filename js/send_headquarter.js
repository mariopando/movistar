...
function template_functions(){
	...
	/* ---------- Inner table content ---------- */
        ...
        $('#input_send_headquarter').focus();
        $('#input_send_headquarter').bind('change',function(e){
            e.preventDefault();
            ola = $(this).val();
            if(ola.length > 0){
                input_select = '<select name="send_headquarter_status"><option selected="selected" value="1">Devolver casa matriz</option><option value="0">Devolver</option></select>';
                input_ola_id = '<input type="hidden" value="'+ola+'" />';
                $('#input_send_headquarter_content > table > tbody').prepend('<tr><td><input type="checkbox" selected="selected" class="row_selected" /></td><td>'+ola+'</td><td>'+input_select+'</td>'+input_ola_id+'</tr>');
                $('#input_send_headquarter').val('');
                $('#input_send_headquarter').focus();
            }else{
                $('#input_send_headquarter').val('');
                $('#input_send_headquarter').focus();
        ...
        $('#selectall_chk').bind('change',function(){
            ref = $(this);
            if(ref.is(':checked')){
                $('#input_send_headquarter_content > table > tbody').find('input[type=checkbox]').each(function(){
        ...
        $('#change_status').bind('change',function(e){
            e.preventDefault();
            ref = $(this);
            $('#input_send_headquarter_content > table > tbody').find('tr').each(function(){
                if($('input.row_selected',$(this)).is(':checked')){
                    $('select[name=send_headquarter_status]',$(this)).val(ref.val());
                }
             });            
        });
